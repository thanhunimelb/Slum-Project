...
to setup
 ...
 ;; create 2 streets at 2 sides of the site.
  ask patches with [pycor <= -32 and pycor >= -40]
    [ set pcolor gray ]

  ask patches with [pycor <= 40 and pycor >= 32]
    [ set pcolor gray ]

 ;; create two centres
 create-centres 1
    [ set color red set shape "dot" set xcor 0  set ycor -30 set size 5 ]
 create-centres 1
    [ set color red set shape "dot" set xcor 0  set ycor 30 set size 5 ]

 ;; create residents
  create-residents number
       [ set size 1 set shape "person" set desiredcommute random commute set xcor ( (random (80))- 40 ) set ycor ( (random (60))- 30 )  set color (random 100) ]
...
to go
   ...
   if ticks > 0 and ticks mod 20 = 0 [
    increaseresidents     ;; increase number of residents after each 20 ticks
  ...
  if ticks >= ntick [stop]   ;; stop after n of tick
end

to increaseresidents
...
to lookaround
     ;; set desiredcommute commute
       if distance targetcentre > desiredcommute
          [ set toofar? 1 ]
       if distance targetcentre < desiredcommute
          [ set toofar? 0 ]
